...
/*
 * serial status data information
 */
typedef struct
{
	uint8_t  connection;		// connection information
	uint8_t  rss;				// signal RSS
	uint32_t date;				// date (reserved)
	uint32_t time;				// time	(reserved)
	uint32_t reserved1;
	uint32_t reserved2;
}__attribute__((packed)) status_info;
 ...
 * |_____|________|_____|__________|_____|_____|
 *   1B      1B     2B     nBytes     4B    1B
 */
typedef struct
{
